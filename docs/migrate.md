# 🚚 Migrating from Misskey/FoundKey to Hajkey
...
```sh
wget -O mkv13.patch https://git.hajkey.org/hajkey/hajkey/raw/branch/dev/docs/mkv13.patch
wget -O mkv13_restore.patch https://git.hajkey.org/hajkey/hajkey/raw/branch/dev/docs/mkv13_restore.patch
...
git remote set-url origin https://git.hajkey.org/hajkey/hajkey.git
git fetch origin
git stash push
rm -rf fluent-emojis misskey-assets
git switch main # or beta or develop
git pull --ff
wget -O renote_muting.patch https://git.hajkey.org/hajkey/hajkey/raw/branch/dev/docs/renote_muting.patch
...
then quit with `\q`, and restart Hajkey.

Note: Ignore errors of `column "xxx" of relation "xxx" already exists`.

If no other errors happened, your Hajkey is ready to launch!
...
```sh
wget -O fk.patch https://git.hajkey.org/hajkey/hajkey/raw/branch/dev/docs/fk.patch
git apply fk.patch
cd packages/backend
...
You ***cannot*** migrate back to Misskey from Hajkey due to re-hashing passwords on signin with argon2. You can migrate from Hajkey to FoundKey, although this is not recommended due to FoundKey being end-of-life, and may have some problems with alt-text.
