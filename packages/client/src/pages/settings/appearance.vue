	...
	<div class="_formRoot">
		...
		<FormSection>
			...
			>
				...
				<template #caption>{{
					i18n.ts.repliesDepthDescription
				}}</template>
			</FormRange>
			...
			<FormSwitch v-model="replyDividerColorize" class="_formBlock">
				...
				<template #caption>{{
					i18n.ts.replyDividerColorizeDescription
				}}</template>
			</FormSwitch>
		...
		<FormSection>
			...
			<FormSwitch v-model="expandPostAlways" class="_formBlock">
				...
				<template #caption>{{
					i18n.ts.expandPostAlwaysDescription
				}}</template>
			</FormSwitch>
			<FormInput
				v-model="expandPostMaxLines"
				type="number"
				class="_formBlock"
			>
				<template #label>{{ i18n.ts.expandPostMaxLines }}</template>
				<template #prefix
					><i class="ph-list-numbers-bold ph-lg"></i
				></template>
				<template #caption>{{
					i18n.ts.expandPostMaxLinesDescription
				}}</template>
			</FormInput>
			<FormInput
				v-model="expandPostMaxCharacters"
				type="number"
				class="_formBlock"
			>
				<template #label>{{
					i18n.ts.expandPostMaxCharacters
				}}</template>
				<template #prefix><i class="ph-hash-bold ph-lg"></i></template>
				<template #caption>{{
					i18n.ts.expandPostMaxCharactersDescription
				}}</template>
			</FormInput>
		</FormSection>
...
const replyDividerColorize = $computed(
	defaultStore.makeGetterSetter("replyDividerColorize")
);
const replyDividerColorGrad = $computed(
	defaultStore.makeGetterSetter("replyDividerColorGrad")
);
const replyDividerColorBg = $computed(
	defaultStore.makeGetterSetter("replyDividerColorBg")
);
const replyDividerColorBorder = $computed(
	defaultStore.makeGetterSetter("replyDividerColorBorder")
);
const replyIndentCompact = $computed(
	defaultStore.makeGetterSetter("replyIndentCompact")
);
const replyCollapseSingles = $computed(
...
const expandPostAlways = $computed(
	defaultStore.makeGetterSetter("expandPostAlways")
);
const expandPostMaxLines = $computed(
	defaultStore.makeGetterSetter("expandPostMaxLines")
);
...
const defaultTimeline = $computed(
	defaultStore.makeGetterSetter("defaultTimeline")
);
...
const indicateNotification = $computed(
	defaultStore.makeGetterSetter("indicateNotification")
);
const indicateMessaging = $computed(
	defaultStore.makeGetterSetter("indicateMessaging")
);
const indicateFollows = $computed(
	defaultStore.makeGetterSetter("indicateFollows")
);
const indicateAnnouncements = $computed(
	defaultStore.makeGetterSetter("indicateAnnouncements")
);
const indicateAntennas = $computed(
	defaultStore.makeGetterSetter("indicateAntennas")
);
const indicateChannels = $computed(
	defaultStore.makeGetterSetter("indicateChannels")
);

const catAvatarRounded = $computed(
	defaultStore.makeGetterSetter("catAvatarRounded")
);
