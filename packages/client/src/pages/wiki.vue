<template>
	<section v-if="mode === 'new' || mode === 'edit'">
		<h1>
			{{ mode === "new" ? i18n.ts._wiki.create : i18n.ts._wiki.edit }}
		</h1>
			...
			<div class="_formRoot">
				<FormSection>
					...
					<FormInput v-model="page.title" class="_formBlock">
						<template #caption>{{
							i18n.ts._wiki.titleDescription
						}}</template>
					</FormInput>
				</FormSection>
				<FormSection>
				...
				<FormSection>
					...
					<FormTextarea v-model="page.content" class="_formBlock">
						<template #caption>{{
							i18n.ts._wiki.contentDescription
						}}</template>
					</FormTextarea>
				</FormSection>
				<FormSection>
					...
					<FormSelect v-model="page.permission" class="_formBlock">
						<option value="public">
							{{ i18n.ts._wiki.public }}
						</option>
						<option value="private">
							{{ i18n.ts._wiki.private }}
						</option>
						<option value="specified">
							{{ i18n.ts._wiki.specified }}
						</option>
					</FormSelect>

					<FormInput
						v-if="permission === 'specified'"
						v-model="page.specified"
						type="textarea"
						class="_formBlock"
					>
						<template #caption>{{
							i18n.ts._wiki.specifiedDescription
						}}</template>
					</FormInput>
				</FormSection>
			</div>
	...
	.use(mdContainer, "spoiler", {
		render: (tokens, idx) => {
			if (tokens[idx].nesting === 1) {
				return `<details class="spoiler"><summary>${tokens[
					idx
				].info.trim()}</summary>\n`;
			} else {
				return "</details>\n";
