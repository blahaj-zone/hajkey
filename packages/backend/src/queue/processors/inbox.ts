...
setInterval(() => {
	...
	if (prevCount > 0) {
		...
		const avgPerHour = (newCount / 60 / 60).toFixed(2);
		const avgPerHourTotal = (totalCount / (runTime / 60 / 60)).toFixed(2);
		...
		statsLog.info(
			[
				...
				`/min: ${avgPerMin}(${avgPerMinTotal}) /hour: ${avgPerHour}(${avgPerHourTotal})`,
